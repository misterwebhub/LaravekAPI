 ...
 * @package App\Imports
 */
class CentreStateImport implements ToCollection, WithHeadingRow
{
    public $data;
            ...
            foreach ($chunk as $row) {
                ...
                if ($row['centre_name']) {
                    ...
                    $centre = Centre::where('name', $row['centre_name'])
                        ...
                        ->first();
                    if ($centre) {
                        if ($centre->state_id != $centreDetails['state_id']) {
                            $centreMismatchState[] = $centre->name;
                        }
                    }
                }
            }
        }
        dd($centreMismatchState);
        $data['status'] = 1;

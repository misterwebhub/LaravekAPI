...
use App\Exports\StudentSuccessErrorExport;
...
class StudentImport implements ToCollection, WithStartRow
{
    ...
    public function __construct($batchId, $approvalConst, $userId)
    {
    ...
    private function fetchdata($row)
    {
        ...
        if (in_array($row[15], ['no', 'n', '0', "", 'No'])) {
            $studentDetails['contactability'] = 0;
        } else {
            $studentDetails['contactability'] = 1;
        }
        $studentDetails['not_contactable_reason'] = trim($row[16]) ?? null;
        $studentDetails['interview1_company_name'] = trim($row[17]) ?? null;
        if (trim($row[18]) != "") {
            if (Carbon::hasFormat($row[18], 'd-m-Y') == false) {
                $studentDetails['interview1_date'] = 1;
            } else {
                $studentDetails['interview1_date'] = Carbon::createFromFormat('d-m-Y', $row[18])->format('Y-m-d');
            }
        } else {
            $studentDetails['interview1_date'] = null;
        }
        $interview1Result = array_search(trim($row[19]), config('staticcontent.interview_result'));
        if ($interview1Result !== false) {
            $studentDetails['interview1_result'] = $interview1Result;
        } else {
            $studentDetails['interview1_result'] = null;
        }
        $studentDetails['interview2_company_name'] = trim($row[20]) ?? null;
        if (trim($row[21]) != "") {
            if (Carbon::hasFormat($row[21], 'd-m-Y') == false) {
                $studentDetails['interview2_date'] = 1;
            } else {
                $studentDetails['interview2_date'] = Carbon::createFromFormat('d-m-Y', $row[21])->format('Y-m-d');
            }
        } else {
            $studentDetails['interview2_date'] = null;
        }
        $interview2Result = array_search(trim($row[22]), config('staticcontent.interview_result'));
        if ($interview2Result !== false) {
            $studentDetails['interview2_result'] = $interview2Result;
        } else {
            $studentDetails['interview2_result'] = null;
        }
        $studentDetails['interview3_company_name'] = trim($row[23]) ?? null;
        if (trim($row[24]) != "") {
            if (Carbon::hasFormat($row[24], 'd-m-Y') == false) {
                $studentDetails['interview3_date'] = 1;
            } else {
                $studentDetails['interview3_date'] = Carbon::createFromFormat('d-m-Y', $row[24])->format('Y-m-d');
            }
        } else {
            $studentDetails['interview3_date'] = null;
        }
        $interview3Result = array_search(trim($row[25]), config('staticcontent.interview_result'));
        if ($interview3Result !== false) {
            $studentDetails['interview3_result'] = $interview3Result;
        } else {
            $studentDetails['interview3_result'] = null;
        }
        if (in_array($row[26], ['no', 'n', '0', "", 'No'])) {
            $studentDetails['placed'] = 0;
        } else {
            $studentDetails['placed'] = 1;
        }
        $monthOfJoining = array_search(trim($row[27]), config('staticcontent.month_of_joining'));
        ...
        if (trim($row[28]) != "") {
            if (Carbon::hasFormat($row[28], 'd-m-Y') == false) {
                $studentDetails['date_of_updation'] = 1;
            } else {
                $studentDetails['date_of_updation'] = Carbon::createFromFormat('d-m-Y', $row[28])->format('Y-m-d');
            }
        } else {
            $studentDetails['date_of_updation'] = null;
        }
        $studentDetails['remarks'] = trim($row[29]) ?? null;
        if (trim($row[3]) != "") {
        ...
        $batchDet = Batch::find($this->batchId);
        if ($batchDet) {
            $centreFromBatch = $batchDet->centre;
            $studentDetails['centre_id'] = $centreFromBatch->id;
            $studentDetails['working_mode'] = $centreFromBatch->working_mode;
            $studentDetails['centre_type'] = $centreFromBatch->centreType->type;
        }
        $studentDetails['phone'] = trim($row[5]) ?? null;
        ...
        $tradeDetails = Trade::where('name', trim($row[6]))->where('type', $studentDetails['centre_type'])->first();
        $studentDetails['trade_course'] = $tradeDetails->id ?? null;
        $studentDetails['updated_email'] = trim($row[55]) ?? null;
        $studentDetails['updated_mobile'] = trim($row[56]) ?? null;
        $studentDetails['batch_id'] = $this->batchId;
    ...
    private function validateFields($id, $studentDetails)
    {
        ...
        // }
        if (!empty($studentDetails['trade_course'])) {
            $centerType = $studentDetails['centre_type'];
        ...
        // if ($studentDetails['placed'] == null) {
        //     $errors[] = trans('admin.placed_invalid');
        // }
    ...
    private function setUser($user, $userDetails, $userId, $password)
    {
        ...
        $centreId = Batch::where('id', $this->batchId)->first()->centre_id;
        $organisationId = Centre::where('id', $centreId)->first()->organisation_id;
    ...
    private function exportData($finalResultAll, $finalResultErrors, $errorCount, $placementSuccess)
    {
        ...
        $uniqid = Str::random();
        $errorFileName = 'student_downlods/error/' . 'error_' . $uniqid . '.csv';
        $fileName = 'student_downlods/' . $uniqid . '.csv';
        Excel::store(new StudentSuccessErrorExport($export), $fileName, 's3');
        Excel::store(new StudentSuccessErrorExport($errorExport), $errorFileName, 's3');
        $data['status'] = 1;
    ...
    {
        $placementInformation = [];
        foreach ([29, 42, 57] as $placementkey => $placementVal) {
    ...
    private function studentAddOrUpdate($user, $studentDetails, $Id, $statusColumn, $finalRes)
    {
        ...
        if ($Id) {
            ...
            $student->save();
            if (!empty($studentDetails['email'])) {
                $this->setEmailVerify($studentDetails, $user->id);
    ...
    private function setEmailVerify($studentDetails, $userId)
    {
        $emailVerify = new EmailVerify();
        $emailVerify->user_id = $userId;
        $emailVerify->email =  $studentDetails['email'];
        $emailVerify->save();
        return $emailVerify;
    }
    ...
    private function userNotExistValidation($studentDetails)
    {
        ...
        $workingMode = $studentDetails['working_mode'];
        if ($workingMode == 0) {
